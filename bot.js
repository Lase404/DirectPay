...
bot.action(/generate_wallet_(.+)/, async (ctx) => {
  ...
  try {
    ...
    // Send Pending Message with Progress Indicator
    const userDoc = await db.collection('users').doc(userId).get();
    const userFirstName = userDoc.data().firstName || 'Valued User';

    // Create Transaction Document with Status 'Processing'
    const transactionRef = await db.collection('transactions').add({
      userId,
      walletAddress,
      chain: chain || 'N/A',
      amount: 0, // To be updated upon deposit
      asset: '', // To be updated upon deposit
      transactionHash: '', // To be updated upon deposit
      referenceId: '', // To be updated upon deposit
      bankDetails: null, // To be updated upon bank linking
      payout: 0, // To be updated upon processing
      timestamp: new Date().toISOString(),
      status: 'Processing',
      paycrestOrderId: '', // To be updated upon Paycrest order creation
      messageId: generatingMessage.message_id
    });
    ...
    // Notify user with Pending Message
    ...
    // Update Transaction with Pending Message ID
...
bankLinkingScene.action('confirm_bank_yes', async (ctx) => {
  ...
  try {
    ...
    if (ctx.session.processType === 'editing') {
      ...
      // Fetch the transaction related to this linking process
      const transactionsSnapshot = await db.collection('transactions')
        .where('userId', '==', userId)
        .where('walletAddress', '==', selectedWallet.address)
      ...
      if (transactionsSnapshot.empty) {
        await ctx.replyWithMarkdown('✅ Bank account linked successfully.', getMainMenu(true, true));
        logger.warn(`No processing transaction found for user ${userId} on wallet ${selectedWallet.address}`);
      } else {
        const transactionDoc = transactionsSnapshot.docs[0];
        const transactionData = transactionDoc.data();

        // Edit the pending message to the final message
        const finalMessage = `Hello ${userState.firstName},\n\n` +
          ...
          `*Cash amount:* NGN ${transactionData.payout}\n` +
        ...
        try {
          await bot.telegram.editMessageText(userId, transactionData.messageId, null, finalMessage, { parse_mode: 'Markdown' });
          // Update transaction status to 'Completed'
          await db.collection('transactions').doc(transactionDoc.id).update({ status: 'Completed' });
        } catch (error) {
...
// Handle Editing Existing Bank Accounts (Moved outside the confirm_bank_yes handler)
...
// Decline Bank Account Confirmation (Moved outside the confirm_bank_yes handler)
bankLinkingScene.action('confirm_bank_no', async (ctx) => {
  await ctx.replyWithMarkdown('⚠️ Let\'s try again.');
...
// Decline Bank Account Confirmation (Already moved outside)
bankLinkingScene.action('confirm_bank_no', async (ctx) => {
  await ctx.replyWithMarkdown('⚠️ Let\'s try again.');
...
// Handle Cancellation of Bank Linking (Already moved outside)
bankLinkingScene.action('cancel_bank_linking', async (ctx) => {
  await ctx.replyWithMarkdown('❌ Bank linking process has been canceled.');
