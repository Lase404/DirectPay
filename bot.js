...
const bankList = [
  ...
  // ... Add all other banks here with their respective codes, aliases, and Paycrest codes
  {
    ...
    paycrestInstitutionCode: 'WEMANGLA',
  },
  {
...
const chains = {
  ...
  Polygon: {
    ...
    apiUrl:
      'https://api.blockradar.co/v1/wallets/f4fc4dc4-a0d5-4303-a60b-e58ec1fc6d0a/addresses',
    supportedAssets: ['USDC', 'USDT'],
    network: 'Polygon',
    assets: {
  ...
  'BNB Smart Chain': {
    ...
    apiUrl:
      'https://api.blockradar.co/v1/wallets/7a844e91-5740-4589-9695-c74411adec7e/addresses',
 ...
 */
async function createPaycrestOrder(
  userId,
  amount,
  token,
  network,
  ...
  try {
    ...
    const bank = bankList.find(
      (b) => b.name.toLowerCase() === recipientDetails.bankName.toLowerCase()
    );
 ...
 */
async function withdrawFromBlockradar(
  chain,
  assetId,
  address,
  amount,
  ...
  try {
    ...
    logger.error(
      `Error withdrawing from Blockradar: ${
        error.response ? error.response.data.message : error.message
      }`
    );
    throw error;
...
async function generateWallet(chain) {
  try {
    ...
    logger.error(
      `Error generating wallet for ${chain}: ${
        error.response ? error.response.data.message : error.message
      }`
    );
    throw new Error(
...
function generateReceipt(txData) {
  ...
  receiptMessage += `*Exchange Rate:* ₦${exchangeRates[txData.asset] || 'N/A'} per ${
    txData.asset || 'N/A'
  }\n`;
  receiptMessage += `*Date:* ${
    txData.timestamp ? new Date(txData.timestamp).toLocaleString() : 'N/A'
  }\n`;
...
const bankLinkingScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    const bank = bankList.find((b) =>
      b.aliases.includes(bankNameInput)
    );
  ...
  async (ctx) => {
    ...
    try {
      const verificationResult = await verifyBankAccount(
        ctx.session.bankData.accountNumber,
        ctx.session.bankData.bankCode
      );
      ...
      logger.error(
        `Error verifying bank account for user ${userId}: ${error.message}`
      );
      await ctx.replyWithMarkdown(
  ...
  try {
    ...
    await bot.telegram.sendMessage(
      ...
      `🔗 User ${userId} linked a bank account:\n\n` +
        ...
        `*Account Number:* ****${userState.wallets[walletIndex].bank.accountNumber.slice(
          -4
        )}`,
      { parse_mode: 'Markdown' }
    );
    logger.info(
      `User ${userId} linked a bank account: ${JSON.stringify(
        userState.wallets[walletIndex].bank
      )}`
    );
    ...
    logger.error(
      `Error in confirm_bank_yes handler for user ${userId}: ${error.message}`
    );
    await ctx.replyWithMarkdown(
...
const sendMessageScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    if (ctx.message.photo) {
      ...
      try {
        await bot.telegram.sendPhoto(userIdToMessage, fileId, {
          caption: caption,
          parse_mode: 'Markdown',
        });
        await ctx.replyWithMarkdown('✅ Photo message sent successfully.');
        logger.info(
          `Admin ${adminUserId} sent photo message to user ${userIdToMessage}. Caption: ${caption}`
        );
      } catch (error) {
        logger.error(
          `Error sending photo to user ${userIdToMessage}: ${error.message}`
        );
        await ctx.replyWithMarkdown(
      ...
      try {
        ...
        logger.info(
          `Admin ${adminUserId} sent text message to user ${userIdToMessage}: ${messageContent}`
        );
      } catch (error) {
        logger.error(
          `Error sending message to user ${userIdToMessage}: ${error.message}`
        );
        await ctx.replyWithMarkdown(
...
const receiptGenerationScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    let keyboard = userState.wallets.map((wallet, index) => [
      Markup.button.callback(
        `Wallet ${index + 1} - ${wallet.chain}`,
        `select_receipt_wallet_${index}`
      ),
    ]);
    await ctx.reply(
  ...
  async (ctx) => {
    ...
    try {
      ...
      let receiptMessage = `🧾 *Transaction Receipt for Wallet ${
        walletIndex + 1
      } - ${wallet.chain}*\n\n`;
      transactionsSnapshot.forEach((doc) => {
        ...
        receiptMessage += `*Exchange Rate:* ₦${exchangeRates[tx.asset] || 'N/A'} per ${
          tx.asset || 'N/A'
        }\n`;
        receiptMessage += `*Date:* ${
      ...
      logger.error(
        `Error generating receipt for user ${userId}: ${error.message}`
      );
      await ctx.replyWithMarkdown(
...
const isAdmin = (userId) =>
  ADMIN_IDS.split(',').map(id => id.trim()).includes(userId.toString());
...
bot.action(/generate_wallet_(.+)/, async (ctx) => {
  ...
  const generatingMessage = await ctx.replyWithMarkdown(
    `🔄 Generating Wallet for *${chain}*... Please wait a moment.`
  );
  ...
  try {
    ...
    if (userState.wallets.length >= MAX_WALLETS) {
      await ctx.replyWithMarkdown(
        `⚠️ You cannot generate more than ${MAX_WALLETS} wallets.`
      );
...
bot.hears('💼 View Wallet', async (ctx) => {
  ...
  try {
    ...
    const generateWalletPage = (page) => {
      ...
      if (page > 1) {
        navigationButtons.push(
          Markup.button.callback('⬅️ Previous', `wallet_page_${page - 1}`)
        );
      }
      if (page < totalPages) {
        navigationButtons.push(
          Markup.button.callback('Next ➡️', `wallet_page_${page + 1}`)
        );
      }
      navigationButtons.push(
        Markup.button.callback('🔄 Refresh', `wallet_page_${page}`)
      );
    ...
    const { message, inlineKeyboard } = generateWalletPage(
      ctx.session.walletsPage
    );
...
bot.action(/wallet_page_(\d+)/, async (ctx) => {
  ...
  try {
    ...
    if (requestedPage > 1) {
      navigationButtons.push(
        Markup.button.callback('⬅️ Previous', `wallet_page_${requestedPage - 1}`)
      );
    }
    if (requestedPage < totalPages) {
      navigationButtons.push(
        Markup.button.callback('Next ➡️', `wallet_page_${requestedPage + 1}`)
      );
    }
    navigationButtons.push(
      Markup.button.callback('🔄 Refresh', `wallet_page_${requestedPage}`)
    );
...
async function sendBaseContent(ctx, index, isNew = true) {
  ...
  navigationButtons.push(
    Markup.button.callback('🔚 Exit', 'exit_base')
  );
...
bot.action('support_contact', async (ctx) => {
  await ctx.replyWithMarkdown(
    'You can contact our support team at [@your_support_username](https://t.me/your_support_username).'
  );
  ctx.answerCbQuery();
});
...
bot.hears(/💰\s*Transactions/i, async (ctx) => {
  ...
  try {
    ...
    const generateTransactionPage = (page) => {
      ...
      if (page > 1) {
        navigationButtons.push(
          Markup.button.callback('⬅️ Previous', `transaction_page_${page - 1}`)
        );
      }
      if (page < totalPages) {
        navigationButtons.push(
          Markup.button.callback('Next ➡️', `transaction_page_${page + 1}`)
        );
      }
      navigationButtons.push(
        Markup.button.callback('🔄 Refresh', `transaction_page_${page}`)
      );
    ...
    const { message, inlineKeyboard } = generateTransactionPage(
      ctx.session.transactionsPage
    );
    await ctx.replyWithMarkdown(message, inlineKeyboard);
  } catch (error) {
...
bot.action(/transaction_page_(\d+)/, async (ctx) => {
  ...
  try {
    ...
    if (requestedPage > 1) {
      navigationButtons.push(
        Markup.button.callback('⬅️ Previous', `transaction_page_${requestedPage - 1}`)
      );
    }
    if (requestedPage < totalPages) {
      navigationButtons.push(
        Markup.button.callback('Next ➡️', `transaction_page_${requestedPage + 1}`)
      );
    }
    navigationButtons.push(
      Markup.button.callback('🔄 Refresh', `transaction_page_${requestedPage}`)
    );
...
bot.action('settings_generate_wallet', async (ctx) => {
  await ctx.reply('💼 *Generate a New Wallet*\n\nSelect the network:', Markup.inlineKeyboard([
    [Markup.button.callback('Base', 'generate_wallet_Base')],
    [Markup.button.callback('Polygon', 'generate_wallet_Polygon')],
    [Markup.button.callback('BNB Smart Chain', 'generate_wallet_BNB Smart Chain')],
  ]));
...
bot.action('settings_edit_bank', async (ctx) => {
  ...
  // If only one wallet, proceed to edit bank details
  if (userState.wallets.length === 1) {
    ctx.session.walletIndex = 0;
    return ctx.scene.enter('bank_linking_scene');
  }
  ...
  let keyboard = userState.wallets.map((wallet, index) => [
    Markup.button.callback(
      `Wallet ${index + 1} - ${wallet.chain}`,
      `select_edit_bank_wallet_${index}`
    ),
  ]);
  await ctx.reply(
    'Please select the wallet for which you want to edit bank details:',
    Markup.inlineKeyboard(keyboard)
  );
  ctx.answerCbQuery();
});
...
bot.action(/select_edit_bank_wallet_(\d+)/, async (ctx) => {
  const userId = ctx.from.id.toString();
      ...
      try {
        ...
        pendingTransactions.forEach(async (transaction) => {
          ...
          try {
            ...
            const accountName = txData.bankDetails && txData.bankDetails.accountName
              ? txData.bankDetails.accountName
              : 'Valued User';
...
// =================== Support Tutorials ===================
// Already defined earlier as detailedTutorials
...
app.post('/webhook/blockradar', async (req, res) => {
  try {
    ...
    fs.appendFileSync(
      ...
      `${new Date().toISOString()} - ${JSON.stringify(event, null, 2)}\n`
    );
    ...
    if (eventType === 'deposit.success') { // Handle 'deposit.success' event
      ...
      if (!existingTxSnapshot.empty) {
        logger.info(
          `Transaction with hash ${transactionHash} already exists. Skipping.`
        );
        return res.status(200).send('OK');
      }
      // **Duplicate Check End**
