...
// Use Session Middleware BEFORE Any Other Middleware or Handlers
bot.use(session());
...
bot.action(/generate_wallet_(.+)/, async (ctx) => {
  ...
  try {
    ...
    // Initialize ctx.session if it's undefined (Defensive Programming)
    if (!ctx.session) {
      ctx.session = {};
...
// Wallet Generation Button Handler
